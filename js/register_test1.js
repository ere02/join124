...
async function register(event) {
  ...
  try {
    users = await getItem('users') || []; // Get existing users (or initialize if none)
    users.push(user);
    await setItem('users', JSON.stringify(users));
    resetForm();
  } catch (error) {
    ...
    const registerBtn = document.querySelector('register-btn'); // Get the register button element
    if (registerBtn) {
